...
class ManualController extends Controller
{
    ...
    const INDEX_TITLE = 'فاکتور ها (تایید پرداخت دستی)';

    const EDIT_TITLE = 'فاکتور ها (تایید پرداخت دستی) - ویرایش';
    ...
    public function index()
    {
        ...
        return view('factor::admin.manual.index', compact('title', 'routeData', 'dataTable'));
    }

    public function edit(Factor $factor)
    {
        $this->isManualPayFactor($factor);

        $factor->load('manual');

        $title = self::EDIT_TITLE;

        return view('factor::admin.manual.edit', compact('title', 'factor'));
    }
    ...
    {
        return route('admin.factor.manual.data');
    }
    ...
    public function data()
    {
        try {
            $factors = Factor::query()
                ...
                ->has('project')
                ->where('status', FactorStatus::PaidManual)
                ->with([
            ...
            return DataTables::eloquent($factors)
                ...
                ->addColumn('action', function (Factor $factor) {
                    return Helper::btnMaker(BtnType::Warning, route('admin.factor.manual.edit', $factor->id), trans('panel.action.edit'));
                })
    ...
    protected function isManualPayFactor(Factor $factor): void
    {
        if ($factor->status !== FactorStatus::PaidManual) {
            abort(404);
        }
    }
}
