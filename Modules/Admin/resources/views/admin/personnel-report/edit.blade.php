                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="type" id="daily" value="daily" 
                                    {{ $personnelReport->type === 'daily' ? 'checked' : '' }}>
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="type" id="hourly" value="hourly"
                                    {{ $personnelReport->type === 'hourly' ? 'checked' : '' }}>
                                <label class="form-check-label" for="hourly">ساعتی</label>
                            </div>
                        </div>

                        <div id="dailyFields" style="display: {{ $personnelReport->type === 'daily' ? 'block' : 'none' }}">
                            <x-admin.input identify="start_date" title="از تاریخ" :is-date-picker="true" 
                                :old="verta($personnelReport->start_date)->format('Y/m/d')" required />
                            <x-admin.input identify="end_date" title="تا تاریخ" :is-date-picker="true" 
                                :old="verta($personnelReport->end_date)->format('Y/m/d')" required />
                        </div>

                        <div id="hourlyFields" style="display: {{ $personnelReport->type === 'hourly' ? 'block' : 'none' }}">
                            <x-admin.input identify="date" title="تاریخ" :is-date-picker="true" 
                                :old="verta($personnelReport->date)->format('Y/m/d')" required />
                            <x-admin.input type="time" identify="start_time" title="از ساعت" 
                                :old="$personnelReport->start_time" required />
                            <x-admin.input type="time" identify="end_time" title="تا ساعت" 
                                :old="$personnelReport->end_time" required />
                        </div>

                        <x-admin.textarea identify="description" title="توضیحات" :old="$personnelReport->description" />
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_emergency" id="isEmergency" value="1"
                                    {{ $personnelReport->is_emergency ? 'checked' : '' }}>
