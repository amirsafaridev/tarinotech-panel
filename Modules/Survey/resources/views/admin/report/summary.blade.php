...
@section('content')
    ...
    <div class="row">
        <!-- Report actions -->
        @include('survey::admin.report.partials.summary.report-actions')

        <!-- Navigation tabs -->
        @include('survey::admin.report.partials.summary.tabs-navigation')
        ...
        <div class="col-xl-12">
            <div class="tab-content" id="questionTabContent">
                    ...
                    @else
                        @foreach($questionsSummary as $index => $question)
                            <div class="card question-card mb-4" id="question-{{ $question['id'] }}">
                                @include('survey::admin.report.partials.summary.question-header')
                                <div class="card-body">
                                    <div class="row">
                                        @include('survey::admin.report.partials.summary.question-stats')
                                        <div class="col-md-8">
                ...
                <div class="tab-pane fade" id="choice-questions" role="tabpanel" aria-labelledby="choice-tab">
                    @include('survey::admin.report.partials.summary.choice-questions-tab')
                </div>

                <!-- Text questions tab -->
                <div class="tab-pane fade" id="text-questions" role="tabpanel" aria-labelledby="text-tab">
                    @include('survey::admin.report.partials.summary.text-questions-tab')
                </div>

                <!-- Number questions tab -->
                <div class="tab-pane fade" id="number-questions" role="tabpanel" aria-labelledby="number-tab">
                    @include('survey::admin.report.partials.summary.number-questions-tab')
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    ...
    @foreach($questionsSummary as $question)
        @if(in_array($question['type'], [QuestionTypeEnum::Single, QuestionTypeEnum::Multiple]) && isset($question['options']) && count($question['options']) > 0)
            @include('survey::admin.report.partials.script.chart-choice-script')
        @endif
    @endforeach

    // Generate charts for number questions
    @foreach($questionsSummary as $question)
        @if($question['type'] == QuestionTypeEnum::Number && isset($question['number_stats']) && isset($question['number_distribution']) && count($question['number_distribution']) > 0)
            @include('survey::admin.report.partials.script.chart-number-script')
        @endif
    @endforeach
@endsection
